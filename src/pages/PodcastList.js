...
function PodcastList() {
  ...
  const fetchPodcasts = async () => {
    ...
    try {
      ...
      setError("Unable to fetch podcasts. Please try again.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const filteredPodcasts = podcasts.filter((p) => {
    const target = `${p.title} ${p.description} ${p.category}`.toLowerCase();
    ...
    <div className="space-y-6">
      <header className="rounded-2xl border border-blush-200 bg-blush-50/90 p-6 shadow-sm">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h1 className="text-2xl font-semibold text-brand-900">Podcasts</h1>
            <p className="mt-2 text-sm text-brand-700">
              Manage your podcast library. Edit or delete existing podcasts, or add a new one from the dashboard.
            </p>
          </div>
      ...
      {loading ? (
        ...
        <div className="rounded-2xl border border-blush-200 bg-blush-50/90 p-10 text-center text-brand-500 shadow-sm">
          {search
            ? "No podcasts match your search. Try changing the keywords."
            : "No podcasts found. Add your first podcast from the dashboard."}
        </div>
      ) : (
        <div className="grid gap-6 md:grid-cols-2">
          {filteredPodcasts.map((podcast) => (
            <PodcastCard key={podcast._id} podcast={podcast} refresh={fetchPodcasts} />
          ))}
        </div>
      )}
    </div>
